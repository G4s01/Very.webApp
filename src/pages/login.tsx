...
export default function LoginPage() {
  ...
  async function sendSms(mask: string) {
    ...
    try {
      ...
      setMsg(`SMS INOLTRATO a ${mask}. VERIFICA!`);
      setStep('awaitOtp');
    } catch (e) {
      setErr(prettyError(e));
      setSelectedMasked('');
      ...
      <VStack align="stretch" spacing={6}>
        ...
        <Box borderWidth="1px" borderRadius="md" p={4}>
            ...
            <VStack spacing={4} align="stretch">
              ...
              {maskedLines.length > 0 && (
                <Button colorScheme="green" variant="outline" onClick={() => { setChannel('sms'); resetErrors(); }}
                  isDisabled={loading} aria-label="Invia OTP via SMS">
                  SMS
                </Button>
              )}
            </VStack>
          ...
          {step === 'awaitOtp' && (
            <VStack spacing={4} align="stretch">
              <Text fontSize="sm" color="gray.600">
                INVIATO {selectedMasked ? `via SMS su ${selectedMasked}` : 'su EMAIL'}:
              </Text>
              <HStack>
                <Input aria-label="INSERISCI OTP" placeholder="INSERISCI OTP" value={otp}
