...
export default function SessionExpiry({ warnAtSec = 5 * 60, refreshEveryMs = 60_000, onExpire }: Props) {
  ...
  // Robust fetch of /api/auth/session and correct interpretation of exp (seconds vs ms)
  async function fetchSession() {
    try {
      ...
      // claims.exp may be seconds or milliseconds. Normalize to milliseconds:
      const rawExp = j?.claims?.exp;
      let expValueMs: number | null = null;
      if (rawExp != null) {
        const num = Number(rawExp);
        if (!Number.isNaN(num) && Number.isFinite(num)) {
          // heuristic: if num looks like milliseconds (> 1e12) treat as ms, otherwise as seconds
          expValueMs = num > 1_000_000_000_000 ? Math.floor(num) : Math.floor(num * 1000);
        }
      }
      if (!expValueMs) {
        setStatus('unauth');
      ...
      setExpMs(expValueMs);
      const rem = Math.floor((expValueMs - Date.now()) / 1000);
      setRemainingSec(rem);
  ...
  // Silent refresh: tenta /api/auth/refresh quando mancano pochi secondi
  useEffect(() => {
    ...
    async function tryRefresh() {
      ...
      try {
        ...
        // ignore: let normal flow handle expiry
      } finally {
        refreshInProgress.current = false;
      }
    }

    if (remainingSec > 0 && remainingSec <= REFRESH_THRESHOLD) {
      tryRefresh();
    }
